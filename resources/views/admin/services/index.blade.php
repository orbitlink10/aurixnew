...
<div class="flex flex-col gap-4">
    <div class="card p-4 rounded-xl">
        ...
        <form method="POST" enctype="multipart/form-data" action="{{ isset($editing) ? route('admin.services.update', $editing) : route('admin.services.store') }}" class="grid md:grid-cols-2 gap-4">
            @csrf
            @isset($editing)
                @method('PUT')
            @endisset
            ...
            <div>
                ...
                <input type="number" step="0.01" name="base_price" value="{{ old('base_price', $editing->base_price ?? 0) }}" class="w-full bg-white border border-slate-300 rounded px-3 py-2 text-slate-900">
            </div>
            ...
            <div class="md:col-span-2">
                <label class="block text-sm mb-1">Service Image {{ isset($editing) ? '(leave blank to keep current)' : '' }}</label>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                        ...
                        <tr class="border-t border-slate-800">
                            <td class="py-2">
                                @if($service->image_url)
                                    <img src="{{ $service->image_url }}" alt="" class="h-10 w-10 rounded object-cover border border-slate-700">
                                @else
                    ...
                    @empty
                        <tr><td class="py-3 text-slate-500" colspan="6">No services created.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
